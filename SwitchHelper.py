...
def should_withdraw(battle : DoubleBattle, idxBattler):
    ...
    target = battle.opponent_active_pokemon[idxBattler]
    if not target.fainted and target.lastMoveUsed:
        moveData = GameData::Move.get(target.lastMoveUsed)
        moveType = moveData.type
        typeMod = pbCalcTypeMod(moveType, target, battler)
        if Effectiveness.super_effective(typeMod) and moveData.base_damage > 50:
...
def pbChooseBestNewEnemy(idxBattler, party, enemies):
    if not enemies or enemies.length == 0:
        return -1
    best    = -1
    bestSum = 0
    for i in enemies:
      ...
      for m in pkmn.moves:
        if m.base_damage == 0:
            continue
        for b in @battle.battlers[idxBattler].allOpposing:
          bTypes = b.pbTypes(True)
          sum += Effectiveness.calculate(m.type, bTypes[0], bTypes[1], bTypes[2])
      if best == -1 or sum > bestSum
        best = i
        bestSum = sum
    return best
